...
public class PBIL {
    ...
    private double[][][] inputToHiddenWs;
    private double[][][] hiddenToOutputWs;
    ...
    public void RunPBIL(String[] args, int[][] clauses, int numVariables, int numClauses) {
        ...
        Input PBILInputs = new Input("ip_train_complete", "ip", false);
        inputs = PBILInputs.getInputs();
        targets = PBILInputs.getTargets();
        
        for (int i = 0; i < populationSize; i++){
            inputToHiddenWs[i] = initializeWeights()[0];
            hiddenToOutputWs[i] = initializeWeights()[1];
        }
        
        inputToHiddenWs = new double[populationSize][numVariables][numVariables*2];
        hiddenToOutputWs = new double[populationSize][numVariables*2][outputSize];
    ...
    public double[][][] initializeWeights(){
        double weight;
        double[][] inputToHiddenW = new double[numVariables][numVariables*2];
        double[][] hiddenToOutputW = new double[numVariables*2][outputSize];
        ...
        double [][][] weights = new double[2][numVariables*2][numVariables*2];
        weights[0] = inputToHiddenW;
        weights[1] = hiddenToOutputW;
        return weights;
    }
    ...
    public void ReadArguments(String[] args) {
        try {
            ...
            outputSize = Integer.parseInt(args[7]);
    ...
    public FitNode[] GenerateFitnessArray(Population population, int[][] clauses) {
        ...
        Individual indie;
        double fitness;
        double[][] inputToHiddenW;
        double[][] hiddenToOutputW;
        
        // create fitness nodes
        for (int i = 0; i < populationSize; ++i) {
            indie = population.GetIndividual(i);
            fitness = fitnessOf(indie, i);
    ...
    public double fitnessOf(Individual indie, int indieIndex){

         double fitness = 0;
         
         MultiOutputNN fitnessNN = new MultiOutputNN(inputToHiddenWs[indieIndex],hiddenToOutputWs[indieIndex]);
         fitness = fitnessNN.scoreNNPBIL(inputs,targets,1);
         /*
    ...
    public void algorithmDone(Individual best, double fitness, int iteration) {
